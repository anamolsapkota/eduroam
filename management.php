...
<div class="row mt-2">
                ...
                <div class="container mt-4">
                    <form action="" method="GET">
                        <div class="input-group mb-3 mt-4">
                            <input type="text" name="search" required="" value="<?php if($_GET['search']){ echo $_GET['search'];} ?>" class="form-control" placeholder="Search data">
            ...
            if ($_SERVER['PHP_AUTH_USER'] === $authUser && $_SERVER['PHP_AUTH_PW'] === $authPass) {
                ...
                if (isset($_GET['search'])) {
                    ...
                    if (count($results) > 0) {
                        ...
                        echo '</table>';
                        echo '<p>Showing results for <b>' . $_GET['search'] . '</b> | Page <b></i>' . $page . '</i></b> of '. $total_pages .'</p>';
                        ...
                        // Previous page link
                        if ($page > 1) {
                            echo '<a href="?search=' . $_GET['search'] . '&page=' . ($page - 1) . '" class="prev">Previous</a>';
                        }

                        // Display numbered page links
                        for ($i = $startRange; $i <= $endRange; $i++) {
                            $activeClass = ($i == $page) ? 'active' : '';
                            echo '<a href="?search=' . $_GET['search'] . '&page=' . $i . '" class="' . $activeClass . '">' . $i . '</a>';
                        }

                        // Next page link
                        if ($page < $total_pages) {
                            echo '<a href="?search=' . $_GET['search'] . '&page=' . ($page + 1) . '" class="next">Next</a>';
                        }
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr>";
                        echo "<td>" . $row["fullname"] . "</td><td>" . $row["org_email"] . "</td><td>" . $row["created_at"] . "</td>";
                ...
                if(mysqli_num_rows($result) > 0 ) {
                    ...
                    echo '<tbody>';
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr><td>" . $row["fullname"] . "</td><td>" . $row["username"] . "</td><td>" . $row["email"] . "</td><td>". $row["updateby"] . "</td><td>" . $row["updatedate"] . "</td></tr>";
                    }
                    echo '</tbody>';
                    echo '</table>';
                    echo '</div>';
    ...
    <script>
        ...
        document.getElementById('designed-developed').appendChild(attribution);
    });
</script>
